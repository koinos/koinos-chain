 ...
 * class is marked as experimental.  Some quick Googling suggests that if you want avoid open( const std::filesystem::path& p, const std::any& o );
...
void controller_impl::open( const std::filesystem::path& p, const std::any& o )
{
   _state_db.open( p, o );
}
...
void controller::open( const std::filesystem::path& p, const std::any& o )
{
   _my->open( p, o );
}
