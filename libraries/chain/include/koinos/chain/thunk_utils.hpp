...
namespace koinos::chain::detail {
   ...
   inline void set_message_field( google::protobuf::Message& msg, uint32_t index, const std::vector< std::string >& values )
   {
      auto desc = msg.GetDescriptor();
      auto ref = msg.GetReflection();
      auto fd = desc->FindFieldByNumber( index );
   ...
   inline set_message_field( google::protobuf::Message& msg, uint32_t index, const std::vector< T >& values )
   {
      auto desc = msg.GetDescriptor();
      auto ref = msg.GetReflection();
      auto fd = desc->FindFieldByNumber( index );
