...
FC_REFLECT( mira::database::configuration::shared_cache,
   (capacity)
);

FC_REFLECT( mira::database::configuration::write_buffer_manager,
   (write_buffer_size)
);

FC_REFLECT( mira::database::configuration::global,
   (shared_cache)
...
FC_REFLECT( mira::database::configuration::bloom_filter_policy,
   (bits_per_key)
   (use_block_based_builder)
);

FC_REFLECT( mira::database::configuration::block_based_table_options,
   (block_size)
   (cache_index_and_filter_blocks)
   (bloom_filter_policy)
);

FC_REFLECT( mira::database::configuration::base_index,
   (optimize_level_style_compaction)
   (increase_parallelism)
   (block_based_table_options)
);

FC_REFLECT( mira::database::configuration::configuration,
   (global)
   (base)
);
