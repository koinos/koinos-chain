...
namespace fc {
    ...
    template<typename Stream, typename T, size_t N>
    inline auto unpack( Stream& s, fc::array<T,N>& v, uint32_t depth ) -> std::enable_if_t<is_trivial_array<T>>
    { try {
       static_assert( N <= MAX_NUM_ARRAY_ELEMENTS, "number of elements in array is too large" );
       depth++;
       FC_ASSERT( depth <= MAX_RECURSION_DEPTH );
   ...
   template<typename Stream, typename K, typename V>
    ...
    template<typename Stream, typename T, std::size_t S>
    inline auto unpack( Stream& s, std::array<T, S>& value, uint32_t depth )  -> std::enable_if_t<is_trivial_array<T>>
    { try {
       static_assert( S <= MAX_NUM_ARRAY_ELEMENTS, "number of elements in array is too large" );
       depth++;
       FC_ASSERT( depth <= MAX_RECURSION_DEPTH );
       s.read((char*)value.data(), S * sizeof(T));
    } FC_RETHROW_EXCEPTIONS( warn, "std::array<${type},${length}>", ("type",fc::get_typename<T>::name())("length",S) ) }
...
/*
    ...
    template<typename Stream> void unpack( Stream& s,  UInt<256>& n, uint32_t depth = 0 )
    {
       UInt<128> tmp[2];
       unpack( s, tmp[0] );
       unpack( s, tmp[1] );
