...
namespace fc {
   ...
   namespace raw {
    ...
    template<typename Stream, typename T, std::size_t S> inline auto pack( Stream& s, const std::array<T,S>& value ) -> std::enable_if_t<!is_trivial_array<T>>;
    template<typename Stream, typename T, std::size_t S> inline auto unpack( Stream& s, std::array<T,S>& value, uint32_t depth = 0 ) -> std::enable_if_t<is_trivial_array<T>>;
    template<typename Stream, typename T, std::size_t S> inline auto unpack( Stream& s, std::array<T,S>& value, uint32_t depth = 0 ) -> std::enable_if_t<!is_trivial_array<T>>;
    ...
    template<typename Stream, typename T, std::size_t N> inline auto pack( Stream& s, const fc::array<T,N>& v) -> std::enable_if_t<!is_trivial_array<T>>;
    template<typename Stream, typename T, std::size_t N> inline auto unpack( Stream& s, fc::array<T,N>& v, uint32_t depth = 0 ) -> std::enable_if_t<is_trivial_array<T>>;
    template<typename Stream, typename T, std::size_t N> inline auto unpack( Stream& s, fc::array<T,N>& v, uint32_t depth = 0 ) -> std::enable_if_t<!is_trivial_array<T>>;
