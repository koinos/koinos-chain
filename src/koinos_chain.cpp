...
int main( int argc, char** argv )
{
  ...
  try
  {
    ...
    // Load hardfork data (optional)
    if( hardfork_data_file.is_relative() )
      hardfork_data_file = basedir / util::service::chain / hardfork_data_file;

    if( std::filesystem::exists( hardfork_data_file ) )
    {
      std::ifstream hifs( hardfork_data_file );
      std::stringstream hardfork_data_stream;
      hardfork_data_stream << hifs.rdbuf();
      std::string hardfork_json = hardfork_data_stream.str();
      hifs.close();

      google::protobuf::util::JsonStringToMessage( hardfork_json, &hardfork_data, jpo );
      LOG( info ) << "Loaded hardfork data file: " << hardfork_data_file;
    }
    else
    {
