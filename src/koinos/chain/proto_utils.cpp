...
value_type get_field_value( const google::protobuf::Reflection* reflection,
...
value_type get_repeated_field_value( const google::protobuf::Reflection* reflection,
                                     const google::protobuf::Message& message,
                                     const google::protobuf::FieldDescriptor* field_descriptor,
                                     google::protobuf::FieldDescriptor::Type type )
{
  ...
  switch( type )
  {
    ...
    case google::protobuf::FieldDescriptor::TYPE_MESSAGE:
      for( int i = 0; i < len; i++ )
        list.add_values()->mutable_message_value()->PackFrom(
          reflection->GetRepeatedMessage( message, field_descriptor, i ) );
      break;
    case google::protobuf::FieldDescriptor::TYPE_BYTES:
      for( int i = 0; i < len; i++ )
...
value_type get_nested_field_value( execution_context& context,
                                   ...
                                   std::string field_name )
{
  ...
  KOINOS_ASSERT( pool_descriptor,
                 internal_error_exception,
                 "${type} descriptor was not found",
  ...
  for( const auto& segment: field_path )
  {
    ...
    KOINOS_ASSERT( pool_field_descriptor,
                   field_not_found_exception,
                   "unable to find field ${fname}",
                   ( "fname", segment ) );
    ...
    KOINOS_ASSERT( field_descriptor,
                   field_not_found_exception,
                   "unable to find field number ${fnum} on ${type} message",
                   ( "fnum", field_num )( "type", message->GetTypeName() ) );

    if( &segment != &field_path.back() )
    {
      KOINOS_ASSERT( type == google::protobuf::FieldDescriptor::Type::TYPE_MESSAGE,
                     field_not_found_exception,
                     "expected nested field to be within a message" );
