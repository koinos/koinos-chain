...
void controller_impl::apply_block_delta(
  const protocol::block& block,
  const protocol::block_receipt& receipt,
   uint64_t index_to )
{
   uint64_t index_message_interval = std::max( 10000ull, index_to / 1000ull );
   static constexpr std::chrono::seconds time_delta = std::chrono::seconds( 5 );
...
void controller::apply_block_delta( const protocol::block& block, const protocol::block_receipt& receipt, uint64_t index_to )
{
